...
unittest
{
    ...
    static class BadNode : TestFullNode
    {
            ...
            uint max_blocks)
        {
            ...
            foreach (idx1, txs1; blocks)
            {
                foreach (idx2, txs2; blocks[idx1].txs)
                {
                    foreach (idx3, txs3; blocks[idx1].txs[idx2].outputs)
                        blocks[idx1].txs[idx2].outputs[idx3].address = prev_key.address;
                }
            }
            ...
            foreach (idx1, txs1; blocks)
            {
                foreach (idx2, txs2; blocks[idx1].txs)
                {
                    foreach (idx3, txs3; blocks[idx1].txs[idx2].inputs)
                        blocks[idx1].txs[idx2].inputs[idx3].signature = signTx(blocks[idx1].txs[idx2]);
                }
                blocks[idx1].header.merkle_root = blocks[idx1].buildMerkleTree();
            }
